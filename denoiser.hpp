...
private:
    ...
    std::vector<double> alphas_cumprod = {};
    std::vector<double> compvis_sigmas = {};
    ggml_tensor* pred_original_sample  = nullptr;
    ggml_tensor* variance_noise        = nullptr;
...
public:
    ...
    ~DDIMTrailingSampler() {
        ...
        delete pred_original_sample;
        delete variance_noise;
    }

    void sample(denoise_cb_t model, ggml_context* work_ctx, ggml_tensor* x, std::vector<float> sigmas, std::shared_ptr<RNG> rng, int i) override {
        const int eta   = 0;
        const int steps = sigmas.size() - 1;
        if (pred_original_sample == nullptr) {
            pred_original_sample = ggml_dup_tensor(work_ctx, x);
        }
        if (eta > 0 && variance_noise == nullptr) {
            variance_noise = ggml_dup_tensor(work_ctx, x);
        }
...
class TCDSampler : public Sampler {
    // Strategic Stochastic Sampling (Algorithm 4) in
    // Trajectory Consistency Distillation
...
std::shared_ptr<Sampler> get_sampler(sample_method_t method) {
    switch (method) {
        ...
        case TCD:
            return std::make_shared<TCDSampler>();
        default:
            LOG_ERROR("Attempting to sample with nonexisting sample method %i", method);
            abort();
    }
