...
struct SDParams {
    ...
    std::string output_path = "output-stream.png";
...
int main(int argc, const char* argv[]) {
    ...
    sd_sampling_stream_t* sampling_stream = nullptr;
    if (params.mode == TXT2IMG) {
        sampling_stream = txt2img_stream(
            sd_ctx,
            params.prompt.c_str(),
            params.negative_prompt.c_str(),
            params.clip_skip,
        ...
        sd_image_t input_image = {(uint32_t)params.width,
                                  ...
                                  3,
                                  input_image_buffer};
        sampling_stream        = img2img_stream(
            sd_ctx,
            input_image,
            mask_image,
            params.prompt.c_str(),
    ...
    if (sampling_stream == NULL) {
        printf("streaming failed\n");
        free_sd_ctx(sd_ctx);
        return 1;
    }

    for (; sd_sampling_stream_sample(sd_ctx, sampling_stream);) {
    }

    sd_image_t current_image = sd_sampling_stream_get_image(sd_ctx, sampling_stream);

    int upscale_factor = 4;  // unused for RealESRGAN_x4plus_anime_6B.pth
    if (params.esrgan_path.size() > 0 && params.upscale_repeats > 0) {
        upscaler_ctx_t* upscaler_ctx = new_upscaler_ctx(params.esrgan_path.c_str(), params.n_threads);
    ...
    std::string final_image_path = dummy_name + ".png";
    stbi_write_png(final_image_path.c_str(), current_image.width, current_image.height, current_image.channel,
                   current_image.data, 0, get_image_params(params, params.seed).c_str());
